...
function operatorMagicLinkEmail(magicLink: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Operator sign in — Artisans' Stories Platform</title>
</head>
      ...
      <table cellpadding="0" cellspacing="0" border="0" width="560" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:40px 40px 32px;text-align:center;">
            <h2 style="margin:0 0 12px;font-size:22px;color:#3a2e24;font-weight:600;">Platform Operator Sign In</h2>
            <p style="margin:0 0 28px;font-size:15px;color:#7a6852;line-height:1.6;">Click the button below to access the platform operator console. This link expires in 15 minutes and can only be used once.</p>
            <table cellpadding="0" cellspacing="0" border="0" style="margin:0 auto 32px;">
              <tr>
                <td style="background:linear-gradient(135deg,#8B6914 0%,#C9A84C 100%);border-radius:10px;">
                  <a href="${magicLink}" style="display:inline-block;padding:16px 40px;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;letter-spacing:0.04em;font-family:'Helvetica Neue',Arial,sans-serif;">Sign In to Platform</a>
                </td>
              </tr>
            </table>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const callbackUrl = safePlatformCallback(body.callbackUrl);

    if (!email || !email.includes("@")) {
      return NextResponse.json({ error: "Valid email required" }, { status: 400 });
    }

    // Look up the operator. Always return success to avoid user enumeration —
    // an unknown or inactive email is a silent no-op (same posture as admin).
    const operator = await prisma.platformOperator.findUnique({ where: { email } });
    if (!operator || !operator.isActive) {
      return NextResponse.json({ success: true });
    }

    // Rate limit: max 3 tokens per email per 5 minutes. PlatformOperatorToken is
    // platform-global (no tenant), keyed by a secret token.
    const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
    const recentTokens = await prisma.platformOperatorToken.count({
    ...
    await prisma.platformOperatorToken.deleteMany({
    ...
    const cbParam =
      callbackUrl && callbackUrl !== "/platform"
        ...
        : "";
    const magicLink = `${siteUrl}/api/auth/platform/verify?token=${encodeURIComponent(token)}${cbParam}`;

    // NOTE: operator emails are NOT written to EmailLog — EmailLog is a
    // tenant-scoped model and operators have no tenant by design (P9).
    await resend.emails.send({
      from: `Artisans' Stories <${process.env.RESEND_FROM ?? "hello@artisansstories.com"}>`,
      to: [email],
      subject: "Your Artisans' Stories platform sign-in link",
      html: operatorMagicLinkEmail(magicLink),
    });

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("Platform operator magic link error:", err);
