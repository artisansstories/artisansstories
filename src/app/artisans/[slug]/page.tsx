...
export default async function ArtisanProfilePage({ params }: PageProps) {
  ...
  const slv = (artisan.socialLinksVisible as Record<string, boolean> | null) ?? {};
  // Filter social links to only show visible ones (default true when not set)
  ...
  const activeProducts = a.products
    .map((pa: { product: { status: string; id: string; slug: string; name: string; price: number; compareAtPrice: number | null; discountType: "PERCENTAGE" | "FIXED" | null; promoLabel: string | null; promoTheme: string | null; isFeatured: boolean; images: { url: string; urlMedium: string | null; altText: string | null }[]; variants: { id: string }[] } }) => pa.product)
    .filter((p: { status: string }) => p && p.status === "ACTIVE") as { id: string; slug: string; name: string; price: number; compareAtPrice: number | null; discountType: "PERCENTAGE" | "FIXED" | null; promoLabel: string | null; promoTheme: string | null; isFeatured: boolean; images: { url: string; urlMedium: string | null; altText: string | null }[]; variants: { id: string }[] }[];
  ...
  return (
    <>
      ...
      <div style={{ background: "#faf7f2", minHeight: "100vh" }}>
        ...
        {activeProducts.length > 0 && (
          <section style={{ maxWidth: 1200, margin: "0 auto 64px", padding: "0 20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 20 }}>
              {activeProducts.map((product: { id: string; slug: string; name: string; price: number; compareAtPrice: number | null; discountType: "PERCENTAGE" | "FIXED" | null; promoLabel: string | null; promoTheme: string | null; isFeatured: boolean; images: { url: string; urlMedium: string | null; altText: string | null }[]; variants: { id: string }[] }) => (
                <ProductCard
                  key={product.id}
                  product={{
                    id: product.id,
                    ...
                    promoTheme: product.promoTheme ?? null,
        ...
        {/* Social Links */}
        {visibleSocialLinks && Object.values(visibleSocialLinks).some(Boolean) && (
          <div style={{ maxWidth: 900, margin: "0 auto 64px", padding: "0 20px", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: 12 }}>
              {visibleSocialLinks.instagram && (
                <a href={visibleSocialLinks.instagram} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 18px", border: "1.5px solid #ede8df", borderRadius: 8, textDecoration: "none", fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#3a2e24", background: "#fff" }}>
              ...
              {visibleSocialLinks.facebook && (
                <a href={visibleSocialLinks.facebook} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 18px", border: "1.5px solid #ede8df", borderRadius: 8, textDecoration: "none", fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#3a2e24", background: "#fff" }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                  Facebook
                </a>
              )}
              {visibleSocialLinks.tiktok && (
                <a href={visibleSocialLinks.tiktok} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 18px", border: "1.5px solid #ede8df", borderRadius: 8, textDecoration: "none", fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#3a2e24", background: "#fff" }}>
              ...
              {visibleSocialLinks.youtube && (
                <a href={visibleSocialLinks.youtube} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 18px", border: "1.5px solid #ede8df", borderRadius: 8, textDecoration: "none", fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#3a2e24", background: "#fff" }}>
              ...
              {visibleSocialLinks.website && (
                <a href={visibleSocialLinks.website} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 18px", border: "1.5px solid #ede8df", borderRadius: 8, textDecoration: "none", fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#3a2e24", background: "#fff" }}>
                  🌐 Website
                </a>
              )}
            </div>
